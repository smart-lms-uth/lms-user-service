...
public class ActivityService {
    ...
    public void logSystemActivity(Long userId, uth.edu.vn.lms_user_service.entity.ActivityType activityType,
    ...
    public List<ActivityResponse> logActivities(Long userId, List<ActivityRequest> requests,
     ...
     */
    public Page<CourseActivityResponse> getAllCourseStudentActivities(Long studentId, Long courseId, int page,
            int size) {
     ...
     */
    public Page<CourseActivityResponse> getCourseStudentActivitiesByType(Long studentId, Long courseId,
            String activityType, int page, int size) {
        Pageable pageable = PageRequest.of(page, Math.min(size, 100), Sort.by(Sort.Direction.DESC, "timestamp"));
        String courseIdStr = String.valueOf(courseId);
     ...
     */
    public Map<Long, Instant> getCourseStudentsLastAccess(Long courseId) {
        // Get all activities for this course
        // Search by both metadata.courseId and URL pattern for backward compatibility
        String courseIdStr = String.valueOf(courseId);
        String urlPattern = "/courses/" + courseIdStr + "(/|\\?|$)";
        
        List<ActivityLog> activities = activityLogRepository.findByCourseIdOrUrlPattern(courseIdStr, urlPattern);

        // Group by userId and get the most recent timestamp for each
        Map<Long, Instant> lastAccessMap = new HashMap<>();

        // Sort by timestamp desc to ensure we get the latest first
        activities.sort((a, b) -> {
        ...
        for (ActivityLog activity : activities) {
            ...
            if (userId != null && !lastAccessMap.containsKey(userId)) {
                lastAccessMap.put(userId, activity.getTimestamp());
            }
        }
